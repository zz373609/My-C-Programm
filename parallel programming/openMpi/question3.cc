...
//description : parallel code whit omp parallel
//date: 2014/2/6
...
limit = (ture_size/total_thread)*(get_num+1);
}
for(int j = (ture_size/total_thread)*(get_num); j<limit;j++)
	for(int i = 0;i<row;i++){
	int s;
			s = check(v,i,j,q,m);
			#pragma omp critical
...
for(int i = (ture_size/total_thread)*(get_num); i<limit;i++)
	for(int j = ture_size;j<row;j++){
	int w;
			w = check(v,i,j,q,m);
		#pragma omp critical
			if(w>max) 
